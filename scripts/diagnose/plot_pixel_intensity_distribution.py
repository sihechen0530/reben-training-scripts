...
def plot_pixel_intensity_distribution(
    ...
    """
    Plot comparative histogram of pixel intensity distributions after gamma correction.
    
    Args:
        ben_pixels_gamma: BigEarthNet pixel values after gamma correction
        natural_pixels: Natural image pixel values (actual or proxy)
        w_dist_gamma: Wasserstein distance between gamma-corrected BigEarthNet and natural images
    ...
    plt.hist(
        ...
        bins=bins,
        alpha=0.6,
        label='BigEarthNet v2 (Gamma-corrected)',
        density=True,
        color='red',
        edgecolor='black',
        linewidth=0.5,
    ...
    plt.title(
        f"Pixel Intensity Distribution Mismatch (After Gamma Correction)\nWasserstein Distance: {w_dist_gamma:.4f}",
        fontsize=14,
        fontweight='bold',
    )
    ...
    ben_mean = ben_pixels_gamma.mean()
    ben_std = ben_pixels_gamma.std()
    natural_mean = natural_pixels.mean()
    natural_std = natural_pixels.std()
    
    stats_text = (
        f"BigEarthNet (γ-corrected): μ={ben_mean:.2f}, σ={ben_std:.2f}\n"
        f"Natural Images: μ={natural_mean:.2f}, σ={natural_std:.2f}"
    )
    plt.text(
        0.02,
        0.98,
...
def main(
    ...
    # Clip BigEarthNet outliers before calculating distance
    ben_pixels_clipped = np.clip(ben_pixels, 0.0, 1.5)  # Allow slightly >1.0 for specular highlights

    # Apply gamma correction (same as SatelliteNormalizer: x -> clamp[0,1] then x^(1/2.2))
    gamma = 1.0 / 2.2
    ben_pixels_gamma = np.power(np.clip(ben_pixels_clipped, 0.0, 1.0), gamma).astype(np.float32)
    
    # Calculate Wasserstein Distance AFTER gamma correction
    print(f"\nStep 3: Calculating Wasserstein Distance after gamma correction...")
    w_dist_gamma = wasserstein_distance(ben_pixels_gamma, natural_pixels)
    print(f"✓ Real Wasserstein Distance (gamma-corrected): {w_dist_gamma:.4f}")
    print(f"✓ BigEarthMean (γ): {np.mean(ben_pixels_gamma):.2f} vs NaturalMean: {np.mean(natural_pixels):.2f}")
    
    # Print statistics (using gamma-corrected data)
    natural_label = "Natural Images (Proxy)" if is_proxy else "Natural Images (ImageNet)"
    print(f"\nStatistics (after gamma correction):")
    print(f"  BigEarthNet (γ-corrected, from clipped [0.0, 1.5]):")
