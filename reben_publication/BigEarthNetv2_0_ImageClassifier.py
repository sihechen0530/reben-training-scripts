...
class BigEarthNetv2_0_ImageClassifier(pl.LightningModule, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        if self.decoupling_epochs is not None:
            ...
            # Default: Use BCE (standard training)
            self.loss = self.bce_loss
            print(f"Initial loss: BCE (standard training)")
    ...
    def on_train_epoch_start(self):
        ...
        if self.decoupling_epochs is not None:
            ...
            if current_epoch == self.decoupling_epochs and self.current_phase == 1:
                ...
                print(f"Phase 2 activated: {num_trainable:,} trainable parameters out of {total_params:,} total")
                print(f"Note: Optimizer will automatically skip frozen parameters (requires_grad=False)")
